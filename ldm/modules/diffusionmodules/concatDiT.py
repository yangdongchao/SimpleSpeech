...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):# x shape (b,c,t)
...
class ConcatDiT2MLP_one(nn.Module):
    """
    ...
    def __init__(
        ...
        self.spk_embedder = ConditionEmbedder(hidden_size, 1024) # add a speaker embedding
    ...
    def forward(self, x, t, context):
        ...
        spk_emb = context[1].unsqueeze(1) # transfer to sequence
        # print('content_context ', content_context.shape)
        # print('spk_emb ', spk_emb.shape)
        # print('self.c1_embedder ', self.c1_embedder)
        # print('self.spk_embedder ', self.spk_embedder)
        #c1,c2 = context.chunk(2,dim=1) # two 
        c = self.c1_embedder(content_context)  # (N,c_len,hidden_size)
        spk_emb = self.spk_embedder(spk_emb)
        # print('c ', c.shape)
        # print('spk_emb ', spk_emb.shape)
        #c2 = self.c2_embedder(c2)  # (N,c_len,hidden_size)
        # c = torch.cat((c1, c2),dim=1) # 
        extra_len = c.shape[1] + 1 + spk_emb.shape[1] # 
        x = self.proj_in(x)
        x = rearrange(x,'b c t -> b t c')
        x = torch.concat([t, spk_emb, c, x],dim=1)
        x = self.pos_emb(x)
        x = rearrange(x,'b t c -> b c t')
