...
from ldm.modules.new_attention import TemporalTransformer
    ...
    def forward(self, x, emb, context=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer) or isinstance(layer, TemporalTransformer):
                x = layer(x, context)
            else:
                x = layer(x)
        return x
...
class NoChangeConv(nn.Module):
    """
    A layer to replace downsample, this layer will not change feature size.
    :param channels: channels in the inputs and outputs.
    :param use_conv: a bool determining if a convolution is applied.
    :param dims: determines if the signal is 1D, 2D, or 3D. If 3D, then
                 downsampling occurs in the inner-two dimensions.
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        out_channels=None,
        use_conv=False,
        use_scale_shift_norm=False, # False in all configs
        dims=2,
        ...
        self.emb_layers = nn.Sequential(
            ...
            linear(
                emb_channels,
                2 * self.out_channels if use_scale_shift_norm else self.out_channels, # use_scale_shift_norm is False
            ),
        )
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = dropout
        self.channel_mult = channel_mult # channel_mult = [1,2]
        ...
        print("为了加深unet层数，删除了DownSample和Upsample")
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):# ResNet是必然存在的，而Attention则要看ds是否在attention_resolutions中出现，最终Downsample部分会有channel_mult * num_res_blocks个ResBlock
                layers = [
                    ResBlock(
                        ch,
            ...
            # if level == 0:
            if level != len(channel_mult) - 1: # 1次
                out_ch = ch
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        NoChangeConv(ch, conv_resample, dims=dims, out_channels=out_ch)
                    )
                )
                ch = out_ch
                input_block_chans.append(ch)
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)# [N] -> [N,self.model_channels]
        emb = self.time_embed(t_emb)# MLP: [N,self.model_channels] -> [N,context_dim]

        if self.num_classes is not None:# only for class label
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)

        if self.use_extra_film_by_addition:# sentence embedding
        ...
        for module in self.input_blocks: # num of TimestepEmbedSequentials
            h = module(h, emb, context)# 0(conv2d):[N,self.model_channels,10,106],1:[N,self.model_channels,10,106],2:[N,self.model_channels,10,106] 3:[N,self.model_channels,5,53] 4:[N,self.model_channels,5,53] 5:[N,self.model_channels*2,5,53]
            hs.append(h)
