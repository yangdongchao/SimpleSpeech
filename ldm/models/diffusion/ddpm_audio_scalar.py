...
class LatentDiffusion_audio(DDPM):
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.shorten_cond_schedule:  # TODO: drop this option, false
                ...
                noise = self.scalar_free.apply(noise)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        #noise = torch.tanh(noise) # transfer to [-1, 1]
        scalar_noise = self.scalar_free.apply(noise) # transfer the scalared noise
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=scalar_noise) # for forward process
    ...
    def p_sample(self, x, c, t, clip_denoised=False, repeat_noise=False,
        ...
        #noise = torch.tanh(noise) # transfer to [-1, 1]
        noise = self.scalar_free.apply(noise) # transfer the scalared noise
        if noise_dropout > 0.:
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if x_T is None:
            ...
            #img = torch.tanh(img)
            img = self.scalar_free.apply(img)
        else:
            img = x_T
        intermediates = []
        if cond is not None:
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
        ...
        if x_T is None:
            ...
            #img = torch.tanh(img) # transfer to [-1, 1]
            img = self.scalar_free.apply(img) # transfer the scalared noise
        else:
            img = x_T
