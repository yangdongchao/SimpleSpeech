...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None,c_film: list = None):
        ...
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(xc, t) # concat then self-attention
        elif self.conditioning_key == 'crossattn':
            if isinstance(c_crossattn, list):
                #cc = torch.cat(c_crossattn, 1)# transfer the list into tensor shape [b,seq_len,dim]
                cc = c_crossattn[0]
                spk_emb = c_crossattn[1] # now, we set the list includes the content condition and speaker condition
            else:
                cc = c_crossattn
            out = self.diffusion_model(x, t, context=[cc,spk_emb]) # cross attention
