...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in).chunk(2) # 同时跑 uncondition and condition
        ...
        tmp_noise = noise_like(x.shape, device, repeat_noise)
        #tmp_noise = torch.tanh(tmp_noise)
        #tmp_noise = self.scalar_free.apply(tmp_noise)
        #noise = sigma_t * noise_like(x.shape, device, repeat_noise) * temperature
        noise = sigma_t * tmp_noise * temperature
        if noise_dropout > 0.:
